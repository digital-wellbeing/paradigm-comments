...
as_draws_df(fit, variable = c("b_", "sd_", "sigma"), regex = TRUE) %>% 
  transmute(
    ...
    Assessment = b_occ,
    `Passive use` = b_passive_c_w,
    `Active use` = b_active_c_w,
    `Sigma (residual)` = sigma,
    `Sigma (passive use)` = sd_pid__passive_c_w,
    `Sigma (active use)` = sd_pid__active_c_w,
    ) %>% 
  ...
  apa_table(
    caption = "Key parameter estimates",
...
qoi <- spread_draws(
  ...
  pivot_longer(c(Negative, Positive, Null, Undecided)) %>% 
  group_by(name) %>% 
  mean_qi(value) %>% 
  mutate(
  ...
  transmute(
    ...
    Percentage = if_else(Association == "Undecided", "", Percentage)
  )
